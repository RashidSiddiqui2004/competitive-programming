...
void a1()
{
    ...
    // Precompute: from each start index, how many potatoes go in one box,
    // and where the pointer ends up.
    ...
    while (vis[start] == -1)
    {
        vis[start] = order.size();
        order.push_back(start);
        start = nxt[start];
    }

    int cycle_start = vis[start];
    ...
    for (auto k : queries)
    {
        ...
        {
            idx = order[cycle_start + (k - cycle_start) % cycle_len];
        }
        cout << cnt[idx] << "\n";
    }
}

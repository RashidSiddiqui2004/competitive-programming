...
void solve()
{
    ...
    // -1 denotes no query is present there
    ...
    fl(i, q)
    {
        int l, r;
        cin >> l >> r;
        // we need the outermost range only
        queries[l] = max(queries[l], r);
    }

    int median = n / 2;
    bool infirsthalf = query(1, median) >= 1;
    ...
    for (int i = 1; i <= n; i++)
    {
        if (queries[i] != -1 && queries[i]>maxright)
        {
            int curr = query(i, queries[i]);
            if (curr > ans)
            {
                ans = curr;
            }

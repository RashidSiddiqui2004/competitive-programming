...
void solve()
{
    ...
    // pres sized n+2 to be safe if mex becomes n+1
    vector<int> pres(n + 2, 0);
    for (int x : a) {
        if (0 <= x && x <= n + 1) pres[x] = 1;
    }

    int currmex = 0;
    while (currmex <= n + 1 && pres[currmex]) ++currmex;

    // count frequencies of numbers < currmex
    vector<int> freq(max(0ll, currmex), 0);
    for (int x : a) if (0 <= x && x < currmex) ++freq[x];

    // hist[f] = how many p have frequency == f
    vector<int> hist(n + 1, 0);
    for (int f : freq) {
        if (f <= n) ++hist[f];
    }

    // prefix sums: cumul[k] = number of p with freq[p] <= k
    vector<int> cumul(n + 1, 0);
    cumul[0] = hist[0];
    for (int i = 1; i <= n; ++i) cumul[i] = cumul[i - 1] + hist[i];
    ...
    for (int k = 0; k <= n; ++k) {
        ...
        int ans = cumul[k]; // number of p in [0, currmex-1] with freq <= k
        // extra case from original logic
        if (currmex <= (n - k)) ++ans;
        cout << ans << ' ';
    }
    cout << '\n';
